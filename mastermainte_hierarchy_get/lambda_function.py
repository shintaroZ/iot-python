...
def convertResult(result):
    ...
    for i in range(len(result)):
        ...
        # キーが異なる場合にセット
        if (HIERARCHY_ID in beforeKeyTable and beforeKeyTable[HIERARCHY_ID] != result[i][HIERARCHY_ID]):
        ...
        if len(parentTable) == 0:
            parentTable[HIERARCHY_ID] = result[i][HIERARCHY_ID]
            parentTable[HIERARCHY_NAME] = result[i][HIERARCHY_NAME]
            parentTable[HIERARCHY_LEVEL] = result[i][HIERARCHY_LEVEL]
            ...
            # キー項目を退避
            beforeKeyTable[HIERARCHY_ID] = result[i][HIERARCHY_ID]

        # 可変部
        if result[i][EQUIPMENT_ID] is not None:
            childTable = {}
            childTable[EQUIPMENT_ID] = result[i][EQUIPMENT_ID]
            childTable[EQUIPMENT_NAME] = result[i][EQUIPMENT_NAME]
...
def lambda_handler(event, context):
    ...
    LOGGER.info('マスタメンテナンス機能_階層マスタ参照開始 : %s' % event)
    ...
    # マスタselect
    result = rds.fetchall(initCommon.getQuery("sql/m_hierarchy/findbyId.sql")
