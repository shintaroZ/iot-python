...
class LambdaFunctionTest(unittest.TestCase):
    ...
    def test_lambda_handler_001(self):
        ...
        # 事前にマスタ追加
        RDS.execute(initCommon.getQuery("test/sql/m_hierarchy/delete.sql"))
        RDS.execute(initCommon.getQuery("test/sql/m_hierarchy/update01.sql"))
        RDS.commit()
        
        # 実行
        lambda_function.lambda_handler(event, None)

        resultArray = RDS.fetchall(initCommon.getQuery("test/sql/m_hierarchy/findbyId.sql"),
                    {
    ...
    def test_lambda_handler_002(self):
        ...
        # 追加
        RDS.execute(initCommon.getQuery("test/sql/m_hierarchy/update02.sql"))
        RDS.commit()
    ...
    def test_lambda_handler_003(self):
        ...
        # 事前にマスタ追加
        RDS.execute(initCommon.getQuery("test/sql/m_hierarchy/delete.sql"))
        RDS.execute(initCommon.getQuery("test/sql/m_equipment/delete.sql"))
        RDS.execute(initCommon.getQuery("test/sql/m_hierarchy/update01.sql"))
        RDS.execute(initCommon.getQuery("test/sql/m_equipment/upsert.sql"))
        RDS.commit()
