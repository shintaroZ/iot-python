...
def isArgument(event, rds):
    ...
    # 子マスタのデータ定義マスタを検索
    result = rds.fetchone(initCommon.getQuery("sql/m_equipment/findCountById.sql"),
                                {
                                    "hierarchyId" : event[HIERARCHY_ID]
                                })
    if (0 < result["count"]):
        raise Exception ("Missing required request parameters. [設備マスタに階層ID:%sが登録済みの為、削除出来ません]" % event["equipmentId"])
...
def lambda_handler(event, context):
    ...
    LOGGER.info('マスタメンテナンス機能_階層マスタ削除開始 : %s' % event)
    ...
    try:
        # 階層マスタの論理削除
        LOGGER.info("階層マスタの論理削除 [%s]" % (event[HIERARCHY_ID]) )
        rds.execute(initCommon.getQuery("sql/m_hierarchy/update.sql"), createDeleteFlgParams(event, 1))
