...
def initConfig(filePath):
    try:
        ...
        setExpiresInterval(config_ini['url setting']['expires_interval'])
...
# --------------------------------------------------
def createSoundParam(soundId):
    param = {}
    param["p_sound_id"] = soundId
    return param
...
def lambda_handler(event, context):
   ...
   # 初期処理
    ...
    soundResult = rds.fetchone(initCommon.getQuery("sql/sound/findById.sql"),
                                  createSoundParam(soundId))

    #none判定
    if soundResult is None:
        LOGGER.error("ファイル名の取得に失敗しました。")
        return {"message":"ファイル名の取得に失敗しました。"}
        sys.exit()
    else:
        pass
    ...
    createDate.strftime("%Y%m%d") + "/" + soundResult['fileName']
    contentsresult = s3.list_objects(Bucket=BUCKET_NAME, Prefix=key)
