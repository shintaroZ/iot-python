...
def createWhereParam(event):
    ...
    whereStr = ""
    whereArray = []
    if DATA_COLLECTION_SEQ in event:
        whereArray.append(" DATA_COLLECTION_SEQ = %d" % event[DATA_COLLECTION_SEQ])
    if CREATED_DATETIME in event:
        whereArray.append(" CREATED_DATETIME = '%s'" % event[CREATED_DATETIME])
    if FILE_TYPE in event:
        whereArray.append(" FILE_TYPE = %d" % event[FILE_TYPE])
...
def lambda_handler(event, context):
    ...
    LOGGER.info('音ファイル作成履歴参照機能 : %s' % event)
    ...
    # マスタselect
    result = rds.fetchall(initCommon.getQuery("sql/t_soundfile_history/findAll.sql")
